...
export default class build extends SfCommand {
    static description = messages.getMessage('description');
    static summary = messages.getMessage('description');
    static examples = messages.getMessages('examples');
    static aliases = ['cli:latestrc:build'];
    static flags = {
    ...
    async run() {
        ...
        const auth = pushChangesToGitHub
            ? ensureString(new Env().getString('GH_TOKEN') ?? new Env().getString('GITHUB_TOKEN'), 'The GH_TOKEN env var is required to push changes to GitHub. Use the --build-only flag to skip GitHub operations (a manual push will then be needed)')
            : undefined;
        // if the github ref is not provided, the dist tag must be
        const ref = flags['start-from-github-ref'] ?? (await this.distTagToGithubRef(ensureString(flags['start-from-npm-dist-tag'])));
        ...
        if (flags.empty) {
            ...
            if (!bumped.length) {
                throw new SfError('No version changes made. Confirm you are passing the correct dependency and version to --only.');
            }
        }
        else {
        ...
        await this.exec('yarn oclif readme --no-aliases --repository-prefix "<%- repo %>/blob/<%- version %>/<%- commandPath %>"');
        ...
        if (pushChangesToGitHub) {
            ...
            if (!repo.package.packageJson.repository) {
                throw new SfError('The repository field is required in the package.json. This is used to determine the repo owner and name to create the release PR.');
            }
...
> Once your PR is ready to be released, merge it into \`${baseBranch}\`.`;
    ...
    async distTagToGithubRef(distTag) {
